class AddressPop extends PopView
{
	...
	private btnDuihuan:egret.Bitmap;
	...
	public setData(data):void
	{
		...
		var bg = new CustomImage("resource/assets/asyn/address_bg.png",true,()=>{
		...
		if(data.sharelist)
		{
			...
			for(var i = 0;i<len;i++)
			{
				...
				if(num)
				{
					var icon = new CustomImage("resource/assets/asyn/moontype/7/"+(i+1)+".png",true,()=>{
		...
		{
			var moon = new CustomImage("resource/assets/asyn/moontype/2/"+data.ptype+".png",true,()=>{
	...
	private duihuanHandler():void
	{
		...
		// else if(sheng.indexOf("西藏") != -1 || sheng.indexOf("青海") != -1 || sheng.indexOf("新疆") != -1)   //sheng.indexOf("内蒙古") != -1 || 
		// {
		// 	Message.show("西藏、青海和新疆暂不支持兑换收货!");
		// }
		else if(!shi)
		{
		...
		}else
		{
			...
			var confirmAdd:ConfirmAddress = new ConfirmAddress();
