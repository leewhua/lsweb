class TetrisGame extends egret.DisplayObjectContainer
{
	public static  maxW = 500;
	public static  maxH = 1000;

	public static  wNum = 10;
	public static  hNum = 20;
	...
	private boxContainer:egret.DisplayObjectContainer;

	private list;

	private select:ItemBox;

	private stepTimer:egret.Timer;

	private boxList;

	private txtScore:egret.TextField;

	private txtStep:egret.TextField;
	...
	private initData():void
	{
		...
		var item:egret.Bitmap = this.boxList["18_4"];
		if(item)
		{
		...
		var item:egret.Bitmap = this.boxList["18_5"];
		if(item)
		{
			item.texture = RES.getRes("box_red_png");
		}else
		...
		var item:egret.Bitmap = this.boxList["19_4"];
		if(item)
		{
			item.texture = RES.getRes("box_red_png");
		}else
		...
		var item:egret.Bitmap = this.boxList["19_5"];
		if(item)
		{
			item.texture = RES.getRes("box_red_png");
		}else
	...
	private downEndHandler():void
	{
		this.stepTimer.delay = 500;
	}

	private downBox():void
	{
		if(this.select)
		{
			this.select.py ++;
	...
	private listDownMove(index):void
	{
		var len = this.list.length;
		...
		{ 
			...
			for(var j = 0;j<itemLen;j++)
			{
				var item:egret.Bitmap = this.boxList[(i-1)+"_"+j];
				if(item)
				{
	...
	private check(item,list):boolean
	{
